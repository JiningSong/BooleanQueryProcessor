...
public class Index {
	...
	/**
	 * For every stored field, we have a HashMap with tokenIdentifier as key and the stored field as value
	 */
	protected HashMap<String, HashMap<String, String>> storedByDocument = new HashMap<>();
	 ...
	 * @return
	 */
	public HashMap<String, String> getStoredDocuments(final String fieldName) {
		HashMap<String, String> stored = this.storedByDocument.get(fieldName);
		if (stored == null) {
			stored = new HashMap<>();
	...
	public void setStoredByDocument(HashMap<String, HashMap<String, String>> storedByDocument) {
	 ...
	 */
	public Document document(final String tokenIdentifier) {
		Document doc = new Document();
		doc.setTokenIdentifier(tokenIdentifier);
		for(String fieldName: storedByDocument.keySet()) {
			HashMap<String, String> storedData = this.storedByDocument.get(fieldName);
			if (storedData.containsKey(tokenIdentifier)) {
				Field f = new Field(fieldName, storedData.get(tokenIdentifier));
				doc.addField(f);
			}
		}
		return doc;
